...
export class DeviceRestoreE2E<T extends { id: string }> {
  ...
  async execute(
    ...
    const [persisted] = await repository.replaceAll([restored]);

    if (!persisted) {
      throw new Error('Device restore validation failed: persisted data missing');
    }
    if (JSON.stringify(restored) !== JSON.stringify(data)) {
      throw new Error('Device restore validation failed: decrypted data mismatch');
    }
    if (JSON.stringify(persisted) !== JSON.stringify(data)) {
      throw new Error('Device restore validation failed: persisted data mismatch');
    }
