...
async function getBootState() {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => resolve(
      request.result ?? { id: BOOT_STATE_ID, ready: false, bootCount: 0 }
    );
...
async function startRuntime() {
  ...
  if (runtimeStatus) runtimeStatus.textContent = state.ready ? 'ready' : 'starting';
...
startRuntime().catch(() => {
  const runtimeStatus = document.querySelector('#runtime-status');
  if (runtimeStatus) runtimeStatus.textContent = 'error';
});
